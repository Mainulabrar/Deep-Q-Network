...
def MinimizeDoseOAR_dvh(MPTV, MBLA, MREC, tPTV, tBLA, tREC, lambdaPTV, lambdaBLA, lambdaREC, VPTV, VBLA, VREC, xVec,gamma,pdose,maxiter):
    ...
    struct xVecAll{
 //       double *new,*old,*xVecPTV1,*xVecPTV2, *xVecBLA, *xVecREC, *xVecR, *xVecP, *xVecAP;
       int nrows;
    };
    struct dosePTV{
        double *d, *y, *dy_sorted, *gamma;
        int *dy_indices, posi;
    };
    struct doseOAR{
        double *d;
    ...
    struct para{
        double lamPTV,lamBLA,lamREC,vPTV,vBLA,vREC,tPTV,tBLA,tREC;
    };
    extern "C"{
    __global__ void dotsortDose(double *data,int *indices, int *indptr, int nrows,double *xVec,double *d,int flag, double *d_sorted)
    {
    ...
    __global__ void scaleDose(int nrows, double *xVec, int posi, double *d_sorted,double pdose, int flag, int nrows2, double *d, double *y,double *xVec_old)
    {
    ...
    __global__ void nrowOverDose(int nrows, int *d_indices, int *rowShort, double *d,double posD)
    {
    ...
    __global__ void overDose(double *r,double *r2,int flag, int nrows,int ncols, int *ncol2,int *d_indices,double *data,int *indices, int *indptr,double *y, double *d,double lam,double t)
    {
    ...
    __global__ void adding(int nrows, double *x, double *y, double *z, double t,int flag,double *xx)
    {
        int row = blockIdx.x * blockDim.x + threadIdx.x;
       // if (row==0)
    ...
    __global__ void copying(int nrows, double *x, double *y)
    {
        int row = blockIdx.x * blockDim.x + threadIdx.x;
        if (row<nrows)
        {
    ...
    __global__ void ydose(int nrows, double *y, double d1,double d2)
    {
        int row = blockIdx.x * blockDim.x + threadIdx.x;
       // if (row==0)
 ...
 ## transfer data from python to GPU with arrays and structures
    class sparseStru:
        def __init__(self,array):
            self.data = gpuarray.to_gpu(array.data.astype(np.double))
    ...
    class xVecAll:
        def __init__(self,xVec):
            ...
            self.new = gpuarray.to_gpu(xVec.astype(np.double))
            self.old = gpuarray.to_gpu(xVec.astype(np.double))
            self.PTV1 =gpuarray.to_gpu(np.empty_like(xVec).astype(np.double))
            self.PTV2 =gpuarray.to_gpu(np.empty_like(xVec).astype(np.double))
            self.BLA = gpuarray.to_gpu(np.empty_like(xVec).astype(np.double))
            self.REC =gpuarray.to_gpu(np.empty_like(xVec).astype(np.double))
            self.R = gpuarray.to_gpu(np.empty_like(xVec).astype(np.double))
            self.P =gpuarray.to_gpu(np.empty_like(xVec).astype(np.double))
            self.AP =gpuarray.to_gpu(np.empty_like(xVec).astype(np.double))
    class dosePTV:
        def __init__(self,gamma):
            self.d = gpuarray.to_gpu(np.empty_like(gamma).astype(np.double))
            self.y =gpuarray.to_gpu(np.empty_like(gamma).astype(np.double))
            self.dy_sorted =gpuarray.to_gpu(np.empty_like(gamma).astype(np.double))
            self.dy_indices = gpuarray.to_gpu(np.empty_like(gamma).astype(np.int32))
            self.y_sorted =gpuarray.to_gpu(np.empty_like(gamma).astype(np.double))
            self.y_indices = gpuarray.to_gpu(np.empty_like(gamma).astype(np.int32))
            self.gamma =gpuarray.to_gpu(gamma.astype(np.double))
            self.posi=np.int32(np.round(0.05*np.double(gamma.shape)))
    class doseOAR:
        def __init__(self,OAR):
            self.d = gpuarray.to_gpu(np.zeros(OAR.shape[0]).astype(np.double))
            self.d_sorted = gpuarray.to_gpu(np.zeros(OAR.shape[0]).astype(np.double))
            self.d_indices =gpuarray.to_gpu(np.zeros(OAR.shape[0]).astype(np.int32))
            self.posi=np.int32(np.round(0.05*np.double(OAR.shape[0])))
    class para:
        def __init__(self,lambdaPTV,lambdaBLA,lambdaREC,VPTV,VBLA,VREC,tPTV,tBLA,tREC):
            self.lamPTV=np.double(lambdaPTV)
            self.lamBLA=np.double(lambdaBLA)
            self.lamREC=np.double(lambdaREC)
            self.vPTV=np.double(VPTV)
            self.vBLA=np.double(VBLA)
            self.vREC=np.double(VREC)
            self.tPTV=np.double(tPTV)
            self.tBLA=np.double(tBLA)
            self.tREC=np.double(tREC)
    ...
    tempD_gpu=gpuarray.to_gpu(np.zeros(num).astype(np.double))
    tempDzero_gpu=gpuarray.to_gpu(np.zeros(num).astype(np.double))
    pdoseA_gpu=gpuarray.to_gpu((np.ones(num)*pdose).astype(np.double))
    ...
    # define a value to scale the over dose values, since the r is too small represented in double format
    Sca=1000;
 ...
 #   print("nrows= ", MPTV_gpu.nrows,"ThreadNum= ", ThreadNum, "BlockNumPTV=", BlockNumPTV)
  ...
  #  print("PTVdose[0]=", PTV_gpu.d[0], "PTVdose_sorted[0]= ", PTV_gpu.dy_sorted[0])
    func_sPTV(xVec_gpu.nrows, xVec_gpu.new,PTV_gpu.posi,PTV_gpu.dy_sorted,np.double(pdose),np.int32(1), MPTV_gpu.nrows,PTV_gpu.d,PTV_gpu.y,xVec_gpu.old,block = (ThreadNum, 1, 1), grid=(BlockNum1, 1))
    ...
    for iter in range(maxiter):
       ...
       # r=-MPTVT.dot(gamma)
        ...
        func_overDose(xVec_gpu.R,tempDzero_gpu,np.int32(2),MPTVT_gpu.nrows,MPTV_gpu.nrows,rowShort,PTV_gpu.dy_indices,MPTVT_gpu.data,MPTVT_gpu.indices,MPTVT_gpu.indptr,PTV_gpu.gamma,tempDzero_gpu,np.double(-Sca),np.double(1),block=(ThreadNum,1,1), grid=(BlockNum0,1))      
       # r=r+MPTVT.dot((y-DPTV)*beta/y.shape[0])
    #__global__ void overDose(double *r,int flag, int nrows,int ncols, int *ncol2,int *d_indices,double *data,int *indices, int *indptr,double *y, double *d,double lam,double t)
        func_overDose(xVec_gpu.R,xVec_gpu.R,np.int32(2),MPTVT_gpu.nrows,MPTV_gpu.nrows,rowShort,PTV_gpu.dy_indices,MPTVT_gpu.data,MPTVT_gpu.indices,MPTVT_gpu.indptr,PTV_gpu.y,PTV_gpu.d,np.double(beta*Sca),np.double(1),block=(ThreadNum,1,1), grid=(BlockNum0,1))
 #       print("PTV_dmax", PTV_gpu.dy_sorted[-1].get(),pdose*tPTV)
        if PTV_gpu.dy_sorted[-1].get()>pdose*tPTV:
            ...
            func_nrowOver(MPTV_gpu.nrows,PTV_gpu.dy_indices,rowShortP,PTV_gpu.d,np.double(DPTVV), block = (ThreadNum, 1, 1), grid=(BlockNumPTV, 1))

            print("rowShortP",rowShortP)
            #r=r+lambdaPTV*MPTV1T.dot(pdose*tPTV*np.ones((DPTVs.shape)) -DPTVs) / DPTVs.shape
            func_overDose(xVec_gpu.R,xVec_gpu.R,np.int32(1),MPTVT_gpu.nrows,MPTV_gpu.nrows,rowShortP,PTV_gpu.dy_indices,MPTVT_gpu.data, MPTVT_gpu.indices, MPTVT_gpu.indptr,pdoseA_gpu,PTV_gpu.d,np.double(1*Sca),para_gpu.tPTV,block=(ThreadNum,1,1),grid=(BlockNum0,1))
  ...
  #      print("BLA_dmax",BLA_gpu.d_sorted[-1].get(),pdose*tBLA)
        if BLA_gpu.d_sorted[-1].get()>pdose*tBLA:
            ...
            func_nrowOver(MBLA_gpu.nrows,BLA_gpu.d_indices,rowShortB, BLA_gpu.d,np.double(DBLAV), block = (ThreadNum,1,1), grid=(BlockNumBLA, 1))
            print("rowShortB",rowShortB)
           # r = r+lambdaBLA * MBLA1T.dot(pdose*tBLA*np.ones((DBLAs.shape,))-DBLAs) / DBLAs.shape
            func_overDose(xVec_gpu.R,xVec_gpu.R,np.int32(1),MBLAT_gpu.nrows,MBLA_gpu.nrows,rowShortB,BLA_gpu.d_indices,MBLAT_gpu.data, MBLAT_gpu.indices, MBLAT_gpu.indptr, pdoseA_gpu,BLA_gpu.d,np.double(lambdaBLA*Sca),para_gpu.tBLA,block=(ThreadNum,1,1),grid=(BlockNum0,1))
   ...
   #     print("REC_dmax",REC_gpu.d_sorted[-1].get(),pdose*tBLA)
        if REC_gpu.d_sorted[-1].get()>pdose*tREC:
            ...
            func_nrowOver(MREC_gpu.nrows,REC_gpu.d_indices,rowShortR,REC_gpu.d,np.double(DRECV),block = (ThreadNum, 1, 1), grid=(BlockNumREC, 1))

            print("rowShortR",rowShortR)
            #r = r+lambdaREC * MREC1T.dot(pdose*tREC*np.ones((DRECs.shape))-DRECs) / DRECs.shape
            func_overDose(xVec_gpu.R,xVec_gpu.R,np.int32(1),MRECT_gpu.nrows,MREC_gpu.nrows,rowShortR,REC_gpu.d_indices,MRECT_gpu.data,MRECT_gpu.indices,MRECT_gpu.indptr,pdoseA_gpu,REC_gpu.d,np.double(lambdaREC*Sca),para_gpu.tREC,block = (ThreadNum,1,1), grid=(BlockNum0,1))
     ...
     #       print("xVec_gpu.R[i]=",xVec_gpu.R[i],rsold)# this contains the scale factor "Sca"
        ...
        if rsold>1e-10:
            for iloop in range (3):
                ...
                func_overDose(xVec_gpu.AP,tempDzero_gpu,np.int32(2),MPTVT_gpu.nrows,MPTV_gpu.nrows,rowShort,PTV_gpu.dy_indices,MPTVT_gpu.data,MPTVT_gpu.indices,MPTVT_gpu.indptr,tempD_gpu,tempDzero_gpu,np.double(beta),np.double(1),block =(ThreadNum,1,1),grid=(BlockNum0,1))
                if PTV_gpu.dy_sorted[-1].get()>pdose*tPTV:
                 #   temp = MPTV1.dot(p)
        #            print("BlockNum_overDose",BlockNumPO)
                    func_overDose(tempD_gpu,tempDzero_gpu,np.int32(0),MPTV_gpu.nrows,MPTV_gpu.nrows,rowShortP,PTV_gpu.dy_indices,MPTV_gpu.data,MPTV_gpu.indices,MPTV_gpu.indptr,xVec_gpu.P,tempDzero_gpu,np.double(1),np.double(1),block =(ThreadNum,1,1),grid=(BlockNumPTV,1))
        #            Ap = Ap+MPTV1T.dot(temp)* lambdaPTV / MPTV1.shape[0]
                    func_overDose(xVec_gpu.AP,xVec_gpu.AP,np.int32(1),MPTVT_gpu.nrows,MPTV_gpu.nrows,rowShortP,PTV_gpu.dy_indices,MPTVT_gpu.data,MPTVT_gpu.indices,MPTVT_gpu.indptr,tempD_gpu,tempDzero_gpu,np.double(1),np.double(1),block =(ThreadNum,1,1),grid=(BlockNum0,1))

                if BLA_gpu.d_sorted[-1].get()>pdose*tBLA:
         #           print("start Ap for BLA")
         #           temp = MBLA1.dot(p)
          #          print("BlockNum_overDoseBLA",BlockNumBO)
                    func_overDose(tempD_gpu,tempDzero_gpu,np.int32(0),MBLA_gpu.nrows,MBLA_gpu.nrows,rowShortB,BLA_gpu.d_indices,MBLA_gpu.data,MBLA_gpu.indices,MBLA_gpu.indptr,xVec_gpu.P,tempDzero_gpu,np.double(1),np.double(1),block =(ThreadNum,1,1),grid=(BlockNumBLA,1))
          #          Ap = Ap+MBLA1T.dot(temp)* lambdaBLA / MBLA1.shape[0]
                    func_overDose(xVec_gpu.AP,xVec_gpu.AP,np.int32(1),MBLAT_gpu.nrows,MBLA_gpu.nrows,rowShortB,BLA_gpu.d_indices,MBLAT_gpu.data,MBLAT_gpu.indices,MBLAT_gpu.indptr,tempD_gpu,tempDzero_gpu,np.double(lambdaBLA),np.double(1),block =(ThreadNum,1,1),grid=(BlockNum0,1))

                if REC_gpu.d_sorted[-1].get()>pdose*tREC:
                #    print("start Ap for REC")
           #         temp = MREC1.dot(p)
                    func_overDose(tempD_gpu,tempDzero_gpu,np.int32(0),MREC_gpu.nrows,MREC_gpu.nrows,rowShortR,REC_gpu.d_indices,MREC_gpu.data,MREC_gpu.indices,MREC_gpu.indptr,xVec_gpu.P,tempDzero_gpu,np.double(1),np.double(1),block =(ThreadNum,1,1),grid=(BlockNumREC,1))
                    #Ap = Ap+MREC1T.dot(temp)* lambdaREC / MREC1.shape[0]
                    func_overDose(xVec_gpu.AP,xVec_gpu.AP,np.int32(1),MRECT_gpu.nrows,MREC_gpu.nrows,rowShortR,REC_gpu.d_indices,MRECT_gpu.data,MRECT_gpu.indices,MRECT_gpu.indptr,tempD_gpu,tempDzero_gpu,np.double(lambdaREC),np.double(1),block =(ThreadNum,1,1),grid=(BlockNum0,1))
                P=xVec_gpu.P.get()
                AP=xVec_gpu.AP.get()
      ...
      #          for i in range(5):
       #             print("xVec_gpu.AP[i]=",xVec_gpu.AP[i])
               ...
               # xVec = xVec + alpha * p
               # xVec[xVec<0]=0
                func_adding(MPTVT_gpu.nrows,xVec_gpu.new,xVec_gpu.new,xVec_gpu.P,np.double(alpha/Sca),np.int32(1),block =(ThreadNum,1,1),grid=(BlockNum0,1))
               # r = r - alpha * Ap
                func_adding(MPTVT_gpu.nrows,xVec_gpu.R,xVec_gpu.R,xVec_gpu.AP,np.double(-alpha),np.int32(0),block =(ThreadNum,1,1),grid=(BlockNum0,1))
                xVecR=xVec_gpu.R.get()
                ...
                if math.sqrt(rsnew) < 1e-5:
                    break
                #p = r + (rsnew / rsold) * p
                func_adding(MPTVT_gpu.nrows,xVec_gpu.P,xVec_gpu.R,xVec_gpu.P,np.double(rsnew/rsold),np.int32(0),block =(ThreadNum,1,1),grid=(BlockNum0,1))
                rsold = rsnew

        func_dose(MPTV_gpu.data,MPTV_gpu.indices, MPTV_gpu.indptr, MPTV_gpu.nrows,xVec_gpu.new,PTV_gpu.d,np.int32(0),PTV_gpu.dy_sorted,block = (ThreadNum, 1, 1), grid=(BlockNumPTV, 1))
        func_adding(MPTV_gpu.nrows,PTV_gpu.y,PTV_gpu.d,PTV_gpu.gamma,np.double(MPTV.shape[0]/beta),np.int32(2),PTV_gpu.y_sorted,block =(ThreadNum,1,1),grid=(BlockNumPTV,1))
        posi=int(0.05*float(MPTV.shape[0]))
        D95=PTV_gpu.y_sorted[posi].get()
        ycheck=PTV_gpu.y_sorted.get()
        print("posi",posi,"D95_y",D95,"nrows",MPTV_gpu.nrows,)
        if D95<pdose:
             #y[(y>=D95)&(y<pdose)]=pdose
            func_ydose(MPTV_gpu.nrows, PTV_gpu.y,np.double(D95),np.double(pdose),block =(ThreadNum,1,1),grid=(BlockNumPTV,1))
        func_adding(MPTV_gpu.nrows,PTV_gpu.gamma,PTV_gpu.gamma,PTV_gpu.d,np.double(beta/MPTV.shape[0]),np.int32(0),block =(ThreadNum,1,1),grid=(BlockNumPTV,1))
        func_adding(MPTV_gpu.nrows,PTV_gpu.gamma,PTV_gpu.gamma,PTV_gpu.y,np.double(-beta/MPTV.shape[0]),np.int32(0),block =(ThreadNum,1,1),grid=(BlockNumPTV,1))

        xVec=xVec_gpu.new.get()
        xVec_old=xVec_gpu.old.get()
        test=LA.norm(xVec-xVec_old)/LA.norm(xVec_old)
        print( "LA.norm(xVec- xVec_old)", LA.norm(xVec- xVec_old),"LA.norm(xVec)",LA.norm(xVec))#,"LA.norm(xVec_old)",LA.norm(xVec_old),'test',test)
        if LA.norm(xVec-xVec_old)/LA.norm(xVec_old)<5e-3:
            break
    ...
    func_sPTV(xVec_gpu.nrows, xVec_gpu.new,PTV_gpu.posi,PTV_gpu.dy_sorted,np.double(pdose),np.int32(0), MPTV_gpu.nrows,PTV_gpu.d,PTV_gpu.y,xVec_gpu.old,block = (ThreadNum, 1, 1), grid=(BlockNum1, 1))
    converge=1
    if iter == maxiter - 1:
        converge = 0
